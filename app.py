"""
Acme Supply Co - Pricing & Quoting App
Simple application for product selection, quote calculation, and proposal/invoice generation.
Multi-supplier tiered pricing with customizable markup, discounts, and order management.
"""
...
if 'order_discount_preset' not in st.session_state:
    st.session_state.order_discount_preset = "Wholesale Discount (5%)"
...
st.title("Acme Supply Co - Pricing & Quoting App")

# Purpose statement
st.markdown("""
**Welcome to the Acme Pricing App** — This tool helps you create quotes and invoices
for products from multiple suppliers. Select products, customize your order, and generate professional
...
def get_price_for_quantity(product_row, quantity):
    ...
    tier_columns = [
        ...
        {'min': 51, 'max': 100, 'column': 'Unit Cost (51-100)'},
        {'min': 101, 'max': 250, 'column': 'Unit Cost (101-250)'},
        {'min': 251, 'max': 500, 'column': 'Unit Cost (251-500)'},
        {'min': 501, 'max': 1000, 'column': 'Unit Cost (501-1000)'},
        {'min': 1001, 'max': float('inf'), 'column': 'Unit Cost (1000+)'}
    ]
...
def calculate_additional_costs(product_row, quantity, include_labels=False):
    ...
    Branding Setup Fee only applies when labels are selected.
    ...
    if include_labels:
        # Branding Setup Fee (one-time per order) - only when labels are selected
        setup_fee = clean_price(product_row.get('Branding Setup Fee', ''))
        ...
        label_cost_per_label = clean_price(product_row.get('Label Cost per Unit', ''))
        if label_cost_per_label is None:
            label_cost_per_label = 0

        label_minimum_raw = clean_price(product_row.get('Label Minimum Qty', ''))
...
def load_pricing_data():
    """
    Load pricing data from suppliers_demo Google Sheet.
    Structure: Row 1 is empty, Row 2 has headers, data starts Row 3.
    Returns a pandas DataFrame.
    """
    gc = connect_to_sheets()
    sheet = gc.open("suppliers_demo").sheet1
    ...
    # Remove empty rows (where Product ID is empty)
    df = df[df['Product ID'].str.strip() != '']
...
try:
    ...
    st.success(f"Loaded {len(df)} products from suppliers catalog")
...
with col2:
    # Filter products based on supplier selection
    if selected_supplier == "All Suppliers":
        available_products = df["Product Name"].unique().tolist()
    else:
        available_products = df[df["Supplier"] == selected_supplier]["Product Name"].unique().tolist()
...
if selected_supplier == "All Suppliers":
    ...
    product_data = df[(df["Supplier"] == selected_supplier) & (df["Product Name"] == selected_product)].iloc[0]

# Display product details in cleaner layout
st.markdown("##### Product Details")
col1, col2 = st.columns(2)
with col1:
    st.markdown(f"**Supplier:** {product_data['Supplier']}")
    st.markdown(f"**Product ID:** {product_data['Product ID']}")
with col2:
    minimum_qty = product_data.get("Minimum Qty", "N/A")
    st.markdown(f"**Minimum Qty:** {minimum_qty if minimum_qty else 'N/A'}")
    category = product_data.get("Category", "N/A")
    st.markdown(f"**Category:** {category if category else 'N/A'}")
...
include_labels = st.checkbox(
    ...
    value=False,
    key="input_labels",
    help="Custom branding labels for the product. Minimum quantities may apply. Includes branding setup fee."
)
...
if base_price is None:
    st.error("No pricing available for this quantity. Please contact the supplier.")
...
if st.button(button_label, type="primary", use_container_width=True):
    ...
    order_item = {
        ...
        'product_ref': product_data["Product ID"],
        'partner': product_data["Supplier"],
    ...
    for idx, item in enumerate(st.session_state.order_items):
        with st.expander(f"{item['product_name']}  -  {item['quantity']} units @ ${item['total_per_unit']:.2f} each  =  ${item['product_total']:.2f}"):
            ...
            if item.get('is_custom', False):
                ...
                with col1:
                    st.write(f"**Supplier:** {item['partner']}")
                    st.write(f"**Product ID:** {item['product_ref']}")
                    st.write(f"**Quantity:** {item['quantity']}")
...
if len(st.session_state.order_items) == 0:
    ...
    for item in st.session_state.order_items:
        ...
        if item.get('is_custom', False):
            ...
            description = f"Product ID: {item['product_ref']}, Supplier: {item['partner']}"
